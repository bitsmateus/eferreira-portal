...
describe('cláusula de honorários — blocos combináveis', () => {
  ...
  it('só fixos: o bloco 1 dentro do item 3.1, e os itens 3.2 a 3.5 do contrato', () => {
    ...
    expect(html).toContain('3.5. Os pagamentos observarão os meios e as condições previstos no item 3.1')
    expect(html).toContain('Favorecido: SERGIO E. FERREIRA SOCIEDADE INDIVIDUAL DE ADVOCACIA, CNPJ: 67.706.981/0001-68')
    expect(html).toContain('Instituição: Nu Pagamentos S.A. — código 260, Agência: 0001, Conta: 278408077-1')
    expect(html).toContain('Chave Pix (e-mail): financeiro@eferreira.adv.br')
